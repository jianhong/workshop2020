...
```{r eval=FALSE}
library(devtools)
install_github("jianhong/workshop2020")
```
```{r extfilepath}
## input the sample bamFile from the http://rbioconductor.com/
## the data is pre-processed chr1 data of GSM1155959
extfilePath <- system.file("extdata", package="workshop2020", mustWork = TRUE)
dir(extfilePath)
```
...
![library complexity curve](libComp.jpg)
